"""
DNS-GA Tier 5: Rare But Deep

Minimal overhead with maximum foresight depth.
Test if rare but very deep forecasting is effective.
"""
...
def calculate_ga_overhead_evals(g_n, num_iterations, population_size, num_ga_children, num_ga_generations):
    ...
    Competition-GA evolves the ENTIRE population through offspring trees.
    Total offspring per call = population_size * sum(num_children^i for i=1 to num_ga_generations)
    Example: population_size=1024, num_children=2, num_generations=2 → 6,144 evals/call
    """
...
EXPERIMENT_CONFIGS = [
    {
        'name': 'DNS-GA_g1000_gen5_iso0.01',
        'g_n': 1000,
        'num_ga_children': 2,
    ...
    {
        'name': 'DNS-GA_g1500_gen5_iso0.01',
        'g_n': 1500,
        'num_ga_children': 2,
        'num_ga_generations': 5,
        'iso_sigma': 0.01,
...
def run_experiment(config, output_dir):
    ...
    # Calculate GA overhead and net savings
    ga_total_evals, ga_num_calls, ga_evals_per_call = calculate_ga_overhead_evals(
        config['g_n'], FIXED_PARAMS['num_iterations'], FIXED_PARAMS['population_size'],
        config['num_ga_children'], config['num_ga_generations']
    )
    ...
    if convergence_iter is not None:
        ...
        print(f"\nEvaluation Analysis:")
        print(f"  Convergence: iter {convergence_iter}, speedup {eval_savings_pct:.1f}%, net savings {net_eval_savings_pct:.1f}%")
...
def main():
    ...
    output_dir = f"dns_ga_tier5_{timestamp}"
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"\n{'='*80}")
    print(f"DNS-GA Tier 5: Rare But Deep - {timestamp}")
    print(f"{'='*80}")
    ...
    results_file = os.path.join(output_dir, "tier5_results.json")
    with open(results_file, 'w') as f:
        json.dump({'results': results}, f, indent=2)
    
    print(f"\n\n{'='*80}")
    print(f"TIER 5 RESULTS")
    print(f"{'='*80}")
