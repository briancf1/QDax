"""
DNS-GA Tier 4: Aggressive Frequency

Test overhead limits by running Competition-GA very frequently.
Will help determine if there's too much disruption to main search.
"""
...
def main():
    ...
    output_dir = f"dns_ga_tier4_{timestamp}"
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"\n{'='*80}")
    print(f"DNS-GA Tier 4: Aggressive Frequency - {timestamp}")
    print(f"{'='*80}")
    ...
    results_file = os.path.join(output_dir, "tier4_results.json")
    with open(results_file, 'w') as f:
        json.dump({'results': results}, f, indent=2)
    
    print(f"\n\n{'='*80}")
    print(f"TIER 4 RESULTS")
    print(f"{'='*80}")
